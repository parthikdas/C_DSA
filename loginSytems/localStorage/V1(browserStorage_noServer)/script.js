...
function loadOrInitializeDataArray(cb) {
    let data =[]
    try{
        data = localStorage.getItem('Data')
        if(data === null) throw 'Data not found new made'
...
function deleteUser(id) {
    loadOrInitializeDataArray(function(data) {
        data.splice(id, 1)
...
document.getElementById('login').addEventListener('click', () => {
    ...
    if(email.value && password.value) {
        loadOrInitializeDataArray(function(data) {
            ...
            if(currentUserId !== -1) { // Existing user
