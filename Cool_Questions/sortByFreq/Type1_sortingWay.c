...
//Question is to sort the array by frequency in decreasing order and if 2 freq is same then put small no first
...
void merge1D(int arr[],int l,int m,int r)
{
    ...
    while(i<n1 && j<n2)//made it ascending, n1 is size of left array, n2 is size of right array
    {
        ...
        else//if ith element in a is larger than jth in b
        {//fill the kth position of arr(original array)
...
void mergeSort1D(int arr[], int l, int r){//dividing array
    if(r>l){
        int m=(l+r)/2;
        mergeSort1D(arr, l, m);
        mergeSort1D(arr, m+1, r);
        merge1D(arr, l, m, r);
    }
}
...
int main(){
	...
	for(i=0;i<n;i++){//filup 2d array, 1st col will store distinct elements and 2nd one will store frequency
		 b[k][0]=a[i];//1st column filup
		int count=1;//initialized with 1 because we already found 1 element
		while(a[i]==a[i+1]){//increment till we are getting the previous number
			count++;i++;
		}
	...
	for(i=0;i<k;i++){//Again printing the array in 1D format
		for(j=0;j<b[i][1];j++){//as 2nd column was storing the frequency, we will print that many times
			printf("%d ", b[i][0]);//1st column was storing the distinct numbers
		}
	 }
	return 0;
}
