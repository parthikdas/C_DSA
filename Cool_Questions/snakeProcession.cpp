...
Input
The first line contains a single integer, R
, which denotes the number of reports to be checked. The description of each report follows after this.
The first line of each report contains a single integer, L
, the length of that report.
The second line of each report contains a string of length L
. The string contains only the characters '.', 'H', and 'T'.
Output
...
#include <string.h>
using namespace std;
string ans(char *s, int n){
    int flag=1;
    for(int i=0;i<n;i++){
        if(s[i]=='H' && flag) flag--;
        else if(s[i]=='H' && !flag) return "Invalid";
        else if(s[i]=='T' && !flag) flag++;
        else if(s[i]=='T' && flag) return "Invalid";
    }
